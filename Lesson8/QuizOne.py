...
def fix_machine(debris, product):
    ...
    productPacer = 0
    letterCounter = len(product)
    falseText = "Give me something that's not useless next time."
    
    while productPacer < letterCounter:
        if debris.find(product[productPacer]) > -1:
            productPacer = productPacer + 1
        else:
            return falseText
    return product
