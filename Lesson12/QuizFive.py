...
def total_enrollment(pool):
    ...
    for university in pool:
        for data in university:
            if k == 1:
                studentEach += university[1]
            elif k == 2:
                tuitionEach += university[2]
            k += 1
        tuitionEach *= studentEach
        studentSum += studentEach 
        tuitionSum += tuitionEach
        tuitionEach = 0
        studentEach = 0
        k = 0
    
    return studentSum,tuitionSum
